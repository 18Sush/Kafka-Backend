	...
	if (file != null && !file.isEmpty()) {
        try {
            ...
            if (currentTimeMillis >= expirationTimeInMillis) {
                ...
                Message message = new Message(null, sender, receiver, content, content, type, null, content);
        ...
        if (currentTimeMillis >= expirationTimeInMillis) {
            ...
            Message message = new Message(null, sender, receiver, content, null, null, null, content);
            message.setSender(sender);
            message.setReceiver(receiver);
            message.setTimestamp(LocalDateTime.now().toString());
...
//  
    ...
    public ResponseEntity<String> resubmitMessage(@PathVariable Long messageId) {
        ...
        if (optionalDeadLetterMessage.isPresent()) {
            ...
            // You can add logging here to check the values
            ...
            // Create a new Message object from the DeadLetterMessage
            Message message = new Message(sender, receiver, content, fileContent, System.currentTimeMillis());
